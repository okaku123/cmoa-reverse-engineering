...
export default function Phase1Screen(props) {
    ...
    async function searchCandidateClipImgs(currentSelectedClipImg) {
        ...
        const currentSelectedClipImgIndex = clipImgsCopy.findIndex(item => item.id == currentSelectedClipImg.id)
        clipImgsCopy.splice(currentSelectedClipImgIndex, 1)
        let clipImgsCopyEntropys = clipImgsCopy.map(item => item.entropy.b)
        let { index: indexofEntropys, closestNumber } = findClosestNumber(clipImgsCopyEntropys, currentSelectedClipImg.entropy.t)
        const firstCandidateClipImgID = clipImgsCopy[indexofEntropys].id
        const firstCandidateClipImg = clipImgsCopy.find(item => item.id == firstCandidateClipImgID)

        clipImgsCopy.splice(indexofEntropys, 1)
        clipImgsCopyEntropys = clipImgsCopy.map(item => item.entropy.b)
        let { index: indexofEntropysNear, closestNumberNear } = findClosestNumber(clipImgsCopyEntropys, currentSelectedClipImg.entropy.t)
        const secondCandidateClipImgID = clipImgsCopy[indexofEntropysNear].id
        const secondCandidateClipImg = clipImgsCopy.find(item => item.id == secondCandidateClipImgID)

        clipImgsCopy.splice(indexofEntropysNear, 1)
        clipImgsCopyEntropys = clipImgsCopy.map(item => item.entropy.b)
        let { index: indexofEntropysNearNear, closestNumberNearNear } = findClosestNumber(clipImgsCopyEntropys, currentSelectedClipImg.entropy.t)
        const thirdCandidateClipImgID = clipImgsCopy[indexofEntropysNearNear].id
        const thirdCandidateClipImg = clipImgsCopy.find(item => item.id == thirdCandidateClipImgID)
    ...
    async function searchCandidateBottomClipImgs(currentSelectedClipImg) {
        ...
        let { index: indexofEntropys, closestNumber } = findClosestNumber(clipImgsCopyEntropys, currentSelectedClipImg.entropy.b)
        const firstCandidateClipImgID = clipImgsCopy[indexofEntropys].id
        const firstCandidateClipImg = clipImgsCopy.find(item => item.id == firstCandidateClipImgID)

        clipImgsCopy.splice(indexofEntropys, 1)
        clipImgsCopyEntropys = clipImgsCopy.map(item => item.entropy.t)
        let { index: indexofEntropysNear, closestNumberNear } = findClosestNumber(clipImgsCopyEntropys, currentSelectedClipImg.entropy.b)
        const secondCandidateClipImgID = clipImgsCopy[indexofEntropysNear].id
        const secondCandidateClipImg = clipImgsCopy.find(item => item.id == secondCandidateClipImgID)

        clipImgsCopy.splice(indexofEntropysNear, 1)
        clipImgsCopyEntropys = clipImgsCopy.map(item => item.entropy.t)
        let { index: indexofEntropysNearNear, closestNumberNearNear } = findClosestNumber(clipImgsCopyEntropys, currentSelectedClipImg.entropy.b)
        const thirdCandidateClipImgID = clipImgsCopy[indexofEntropysNearNear].id
        const thirdCandidateClipImg = clipImgsCopy.find(item => item.id == thirdCandidateClipImgID)
     ...
     * @param {Int} row 
     */
    async function reCorpClipImgs(offset) {
        ...
        //修改确认组里最后一排的切片偏移量
        let bottomClip = confirmClipImgs[0].content[confirmClipImgs[0].content.length - 1][0]
        ...
        blockCtx.drawImage(img0, offsetX, offsetY, clipWidth, clipHeight, 0, 0, clipWidth, clipHeight);
        var imageData_b = blockCtx.getImageData(0, clipHeight - calcHeight, clipWidth, calcHeight);
        ...
        var imageData_r = blockCtx.getImageData(0, clipWidth - calcHeight, calcHeight, clipHeight);
        var entropy_r = calculateEntropy(imageData_r.data);
        var blockDataURL = blockCanvas.toDataURL();

        const index = unConfirmClipImgs.findIndex(item => item.originIndex == bottomClip.originIndex)
        unConfirmClipImgs[index].entropy = { l: entropy_l, r: entropy_r, t: entropy_t, b: entropy_b }
        ...
        unConfirmClipImgs[index].coordinate = bottomClip.coordinate
         //修改确认组里最后一排的切片偏移量
        confirmClipImgs[0].content[  confirmClipImgs[0].content.length - 1 ][0] = lodash.cloneDeep(unConfirmClipImgs[index])
    ...
    function hande(event) {
        ...
        //修改确认组里最后一排的切片偏移量
        let firstItem = confirmClipImgs[0].content[confirmClipImgs[0].content.length - 1][0]
    ...
    async function getImgClipTopPartEntropy(preImgClip) {
        ...
        var blockHeight = 368
        //把切片画出来
        blockCtx.drawImage(img0, 0, 0, blockWidth, blockHeight, 0, 0, blockWidth, blockHeight);
    ...
    function findSingoPair(index = 0) {
        ...
        let { index: indexofEntropys, closestNumber } = findClosestNumber(entropys, sourceClipTopEntropy)
    ...
    function confirmTopClip(clipImg) {
        ...
        const index = tempClipImgs.findIndex(item => item.id == currentSelectedClipImg.id)
        ...
        const checkedClipImgs = tempClipImgs.filter(item => (item.confirm.t || item.confirm.b))
        const unCheckedClipImgs = tempClipImgs.filter(item => (!item.confirm.t && !item.confirm.b))
    ...
    function confirmBottomClip(clipImg) {
        ...
        if (clipImg.confirm.b) {
            ...
            confirmClipImg.content.unshift([lodash.cloneDeep(currentSelectedClipImg)])
            confirmClipImgs[index] = confirmClipImg
            setConfirmClipImgs(Array.from(confirmClipImgs))
    ...
    function unPiarClipImg(confirmClipImg, index) {
        ...
        if (type == "1x2") {
            ...
            let topClipIndex = tempClipImgs.findIndex(item => item.id == topClip.id)
            tempClipImgs[topClipIndex] = topClip

            let bottomClipIndex = tempClipImgs.findIndex(item => item.id == bottomClip.id)
            tempClipImgs[bottomClipIndex] = bottomClip

        } else {
            let bottomClip = confirmClipImg.content.at(confirmClipImg.content.length - 1).at(0)
            let ThirdClip = confirmClipImg.content.at(confirmClipImg.content.length - 2).at(0)
            bottomClip.confirm.t = null
            ThirdClip.confirm.b = null

            let topClipIndex = tempClipImgs.findIndex(item => item.id == ThirdClip.id)
    ...
    function confirmLargeLeftClip(pair) {
        ...
        if (~confirmLargeClipImgIndex) {
            ...
            newPiar.content = [[tr, tl], [br, bl]]

            const index = confirmClipImgs.findIndex(item => currentSelectedPair.id == item.id)
        ...
        <div
            ...
            <div
                ...
                <div className='w-full px-5 flex justify-between items-center'>
                    <lable className="text-sm">剩余切片 {unConfirmClipImgs.length - confirmClipImgs.length * 2}</lable>
                    <lable className="text-sm">匹配切片 {confirmClipImgs.length * 2}</lable>
                </div>
                        ...
                        unConfirmClipImgs.map(clipImg => {
                            ...
                            return (
                                <div
                                    ...
                                    style={{ fontSize: 9 }} className=''>
                                    <div className='text-center'>{entropy.t}</div>

                                    <div className='flex justify-center items-center'>
                                        <label style={{ writingMode: "vertical-lr" }}>{entropy.l}</label>
                                        ...
                                        ></img>
                                        <label style={{ writingMode: "vertical-lr" }} >{entropy.r}</label>
                                    </div>
            ...
            >
                <div
                    ...
                    {confirmClipImgs.map((item, index) => {
                        ...
                        let cellStyle = "flex flex-col justify-start items-center cursor-pointer "
                        if (index === 0) {
                            cellStyle = "flex flex-col justify-start items-center cursor-pointer shrink-0"
                        }
            ...
            <div className="w-1/4 min-h-96 border-l-[1px] border-emerald-300 self-stretch">
                ...
                <div className='w-full text-center mt-5 px-5 flex justify-between items-center'>
                    ...
                    <label className='btn btn-accent btn-sm '
                        onClick={() => {
                    ...
                    {candidateClipImgs.map(item => {
                        ...
                        return (
                            <div
                                ...
                                <div className='flex justify-center items-center'>
                                    <label style={{ writingMode: "vertical-lr" }}>{item.entropy.l}</label>
                                    <img className='w-20' src={item.url}
                                        onClick={() => {
                                            confirmTopClip(item)
                                        }}
                                    ...
                                    ></img>
                                    <label style={{ writingMode: "vertical-lr" }} >{item.entropy.r}</label>
                                </div>
...

                {
                    ...
                    <div className='w-full flex justify-center items-center'>
                        <div style={{ fontSize: 9 }} className=''>
                            <div className='text-center'>{currentSelectedClipImg.entropy.t}</div>
                            <div className='flex justify-center items-center'>
                                <label style={{ writingMode: "vertical-lr" }}>{currentSelectedClipImg.entropy.l}</label>
                                <img className='w-20' src={currentSelectedClipImg.url}></img>
                                <label style={{ writingMode: "vertical-lr" }} >{currentSelectedClipImg.entropy.r}</label>
                            </div>
                            <div className='text-center' >{currentSelectedClipImg.entropy.b}</div>
                        </div>
