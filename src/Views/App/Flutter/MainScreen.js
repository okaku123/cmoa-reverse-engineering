                ...
                {currentPage == "1" &&
                    <div
                        ...
                        <div className='w-[320px] border-dashed border-2 border-yellow-300 flex justify-between items-start ml-5' >
                            ...
                            {/* 中间的当前选择切片 */}
                            <div className='w-20 flex flex-col justify-center items-center ml-5'
                                style={{ height: "calc(100vh - 176px)" }}
                            >{currentSelectedPair &&

                                <div
                                    Key={"confirmClipImgs" + currentSelectedPair.content.at(0).at(0).id}
                                    style={{ fontSize: 9 }} className=''>
                                    <div className='text-center'>{currentSelectedPair.content.at(0).at(0).entropy.t}</div>
                                    <div className='flex flex-col justify-start items-center cursor-pointer'
                                        onClick={() => {

                                        }}
                                    >
                                        <img className='w-20' src={currentSelectedPair.content.at(0).at(0).url}></img>
                                        <img className='w-20' src={currentSelectedPair.content.at(1).at(0).url}></img>
                                    </div>
                                    <div className='text-center' >{currentSelectedPair.content.at(1).at(0).entropy.b}</div>
                                </div>
                                }
                            </div>
                            {/* 右边的候选切面 */}
                            <div className='w-25 overflow-y-scroll  overflow-x-hidden'
                                style={{ height: "calc(100vh - 176px)" }}
                            >
                                {candidateLeftClipImgs.map((item, index) => {
                                    let { id, type, content } = item
